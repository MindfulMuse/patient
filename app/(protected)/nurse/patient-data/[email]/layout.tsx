...
export default function NursePatientLayout({ children }: { children: ReactNode }) {
  const params = useParams();
  const decodedEmail = decodeURIComponent(params.email as string); // patient's email
  const [menuOpen, setMenuOpen] = useState(false);

  // Define nurse-specific actions with proper paths
  const actions = [
    {
      label: "Trends",
      href: `/nurse/patient-data/${encodeURIComponent(decodedEmail)}/trends`,
